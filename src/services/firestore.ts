...
export const enregistrerNouvelleMusiqueDansFirestore = async (
  ...
  try {
    ...
    console.log(
      "Impossible d'ajouter à Firestore, ajout en local:",
      erreur.message,
    );
...
export const likerUneChanson = async (songId: string) => {
  try {
...
export const retirerLike = async (songId: string) => {
  try {
    const user = auth().currentUser;
    if (!user) return;
...
export const recupererLikesUtilisateur = async () => {
  try {
    const user = auth().currentUser;
    if (!user) return {};
    ...
    console.log('Erreur récupération likes:', error);
    return {};
  }
};

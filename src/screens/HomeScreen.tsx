...
const EcranAccueil = ({navigation}: any) => {
  ...
  const chargerLikes = async () => {
    const data = await recupererLikesUtilisateur();
    setLikes(data);
  };
  const gererLike = async musique => {
    const estDejaLike = likes[musique.id];

    if (estDejaLike) {
      await retirerLike(musique.id);
    } else {
      await likerUneChanson(musique.id);
    }

    const nouveauxLikes = await recupererLikesUtilisateur();
    setLikes(nouveauxLikes);
  };
  ...
  const gererLectureMusique = async (musiqueSelectionnee: any) => {
    await chargerEtJouerUneListeDeMusiques([
      musiqueSelectionnee,
      ...listeDesMusiques.filter(m => m.id !== musiqueSelectionnee.id),
    ]);
  };
    ...
    <SafeAreaView style={styles.conteneurPrincipal}>
      <ScrollView padding={SPACING.m}>
        ...
        <View style={{marginTop: 30}}>
          <Animated.View
            style={[styles.titreSquelette, {opacity: animationPulsation}]}
          />
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          <View style={styles.sectionEnTete}>
            <View style={styles.ligneFiltres}>
              <TouchableOpacity style={styles.piluleActive}>
                <Text style={styles.textePiluleActive}>Tout</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.pilule}>
                <Text style={styles.textePilule}>Musique</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.pilule}>
                <Text style={styles.textePilule}>Podcasts</Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          <FlatList
            ...
            renderItem={({item}) => (
              <ComposantCarteMusique
                ...
                // ❤️ LIKE
                estLike={!!likes[item.id]}
                onLike={() => gererLike(item)}
              />
            )}
          />
